    ...
    >
      <ol className="flex min-w-0 flex-wrap items-center gap-1">
        {crumbs.map((crumb, index) => {
          ...
          const isPlanNameCrumb = planId != null && index === 1 && crumbs[0]?.href === "/";

          return (
            <Fragment key={`${crumb.label}-${index}`}>
              {index > 0 && (
                <li className="shrink-0 text-gray-600 select-none" aria-hidden>
                  ›
                </li>
              )}
