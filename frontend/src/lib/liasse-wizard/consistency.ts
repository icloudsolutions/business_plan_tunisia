...
export function buildPreflightReport(
  ...
  const alerts = getConsistencyAlerts(values);

  for (const a of alerts) {
    items.push({
      label: a.message,
      detail: a.severity === "error" ? "À corriger avant soumission" : "Recommandation",
      severity: a.severity,
    });
  }
  ...
  for (const path of missingFields) {
    items.push({
      label: `Champ requis : ${path}`,
      detail: "Signalé par le serveur à la dernière sauvegarde",
      severity: "warning",
    });
  }
  ...
  if (items.length === 0) {
    items.push({
      ...
      detail: "Vous pouvez enregistrer et lancer le calcul 7 ans.",
      severity: "ok",
    });
  }
